...
public class StockSender implements Runnable {
    ...
    public void run() {
        ...
        while (true) {
            sentCount++;
            // Create a producer record to send (use function createRandomRecord)
            ...
            try {
                // Send the record and display the record meta data when sendCount % 100 == 0
    ...
    private ProducerRecord<String, StockPrice> createRandomRecord(
            final Random random) {
        final int dollarAmount = randomIntBetween(random, stockPriceHigh.getDollars(), stockPriceLow.getDollars());
        final int centAmount = randomIntBetween(random, stockPriceHigh.getCents(), stockPriceLow.getCents());
        final StockPrice stockPrice = new StockPrice(stockPriceHigh.getName(), dollarAmount, centAmount);
